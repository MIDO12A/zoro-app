...
class _AgencyDashboardScreenState extends State<AgencyDashboardScreen>
  ...
  // ── subscribe realtime بعد معرفة agencyId ───────────────────────────────
  void _bindRealtime(String agencyId) {
    ...
    // ✅ تغييرات دفتر الألماس الموحد (الجدول الصحيح: agency_diamond_ledger)
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (agencyId == null) {
        ...
        // ابدأ الـ Realtime فور معرفة agencyId (guard: widget قد يكون تلف أثناء الـ await)
        if (!mounted) return;
  ...
  Widget _buildError() => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(_error ?? 'خطأ', style: const TextStyle(color: _red,
