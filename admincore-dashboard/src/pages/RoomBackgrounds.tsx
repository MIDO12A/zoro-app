...
export default function RoomBackgrounds() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('هل تريد فعلاً حذف هذه الخلفية؟')) return;
    try {
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-white text-lg font-semibold flex items-center gap-2">
            ...
            خلفيات الغرف (Room Backgrounds)
          </h2>
          <p className="text-slate-500 text-xs mt-0.5">إدارة وإضافة خلفيات الغرف الصوتية ليتمكن المستخدمون من اختيارها.</p>
        </div>
        <button onClick={handleUpload} disabled={uploading} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-bold rounded-lg flex items-center gap-2 transition-colors">
          <Upload className="w-4 h-4" /> {uploading ? 'جاري الرفع...' : 'إضافة خلفية'}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-[#111113] rounded-xl border border-dashed border-white/10 text-slate-500 text-sm">
          لا يوجد خلفيات حالياً. اضغط على زر الإضافة للبدء.
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
          {assets.map(a => (
            <div key={a.id} className="bg-[#1a1a1e] rounded-xl border border-white/5 overflow-hidden group">
              <div className="relative w-full aspect-[9/16] bg-black/40 flex items-center justify-center p-2">
                <img src={a.remoteUrl} className="max-w-full max-h-full object-cover rounded" />
              </div>
